...
import { Star, ShoppingCart, Heart, Zap, ArrowRight, Plus, Minus } from 'lucide-react'
...
const ProductCard = ({ product }: { product: Product }) => {
  ...
  const updateQuantity = (quantity: number) => {
    if (quantity <= 0) {
      ...
      dispatch({ type: 'UPDATE_QUANTITY', payload: { productId: product.id, quantity } })
    }
  }
    ...
    >
      ...
      <div className="relative h-56 bg-gray-100 overflow-hidden">
        ...
        <div className={`absolute bottom-0 left-0 right-0 p-4 transition-all duration-300 ${
          isHovered && cartQuantity === 0 ? 'translate-y-0 opacity-100' : 'translate-y-full opacity-0'
        }`}>
          <button
            onClick={addToCart}
...
export default function FeaturedProducts({ products: propProducts }: FeaturedProductsProps) {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const featured = allProducts.filter((p: Product) => featuredIds.includes(p.id)).slice(0, 4)
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-16">
          <div className="inline-flex items-center gap-2 bg-gradient-to-r from-orange-100 to-red-100 border border-orange-200 rounded-full px-4 py-2 mb-6">
            ...
            <span className="text-sm font-semibold text-orange-700">Featured Products</span>
          </div>
          <h2 className="text-5xl md:text-6xl font-extrabold text-gray-900 mb-4">
            Our <span className="bg-gradient-to-r from-orange-600 to-red-600 bg-clip-text text-transparent">Best Sellers</span>
          </h2>
          <p className="text-xl text-gray-600 max-w-2xl mx-auto">
            Handpicked selection of premium chicken products with exceptional quality
          </p>
        </div>
