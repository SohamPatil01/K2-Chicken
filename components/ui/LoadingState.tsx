...
export function LoadingSpinner({
  ...
  return (
    ...
    >
      <div
        className={`animate-spin rounded-full border-k2-paper border-t-brand-red ${spinnerSizes[size]} mx-auto`}
        aria-hidden
      />
      {showLabel ? (
        <p className="mt-4 text-lg text-[#5a6a61] font-medium text-center">
          {label}
        </p>
      ) : (
...
export function HomeRecipeSectionFallback() {
  return (
    <section
      className="min-h-[300px] py-16 sm:py-20 bg-k2-cream border-t border-k2-paper"
      role="status"
      aria-live="polite"
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {[1, 2, 3].map((i) => (
            <div key={i} className="testimonial-card p-8 animate-pulse">
              <div className="h-4 bg-k2-cream-dark rounded w-1/4 mb-4" />
              <div className="h-20 bg-k2-cream-dark rounded mb-6" />
...
export function HomePromotionsSectionFallback() {
  return (
    <div
      className="min-h-[200px] max-w-[1180px] mx-auto px-4 sm:px-6 lg:px-8 py-6"
      role="status"
      aria-live="polite"
      aria-label="Loading promotions"
    >
      <span className="sr-only">Loading promotions</span>
      <div className="h-36 rounded-card bg-k2-cream-dark animate-pulse border border-k2-paper" />
    </div>
  );
}
...
export function HomeContactSectionFallback() {
  return (
    <section
      className="min-h-[320px] py-16 bg-k2-cream border-t border-k2-paper"
      role="status"
      aria-live="polite"
    ...
    >
      <div className="max-w-[1180px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          ...
          <div className="h-64 lg:h-auto min-h-[200px] bg-gray-200 rounded-card animate-pulse" />
        </div>
      </div>
    </section>
  );
