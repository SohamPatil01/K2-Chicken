...
async function clearOrders() {
  ...
  try {
    ...
    console.log(`   Deleted ${inventoryHistoryResult.rows.length} inventory history record(s) related to orders`);
    
    await client.query('COMMIT');
    
    console.log('✅ Successfully cleared all orders from the database!');
    console.log(`   Total orders deleted: ${ordersResult.rows.length}`);
    console.log(`   Total order items deleted: ${orderItemsResult.rows.length}`);
